...
class AssociationFunction:
    ...
    def centroid_batch(self, bboxes1, bboxes2) -> np.ndarray:
        if len(bboxes1) == 0:
            bboxes1 = np.empty([0, 4])
        else:
            if not isinstance(bboxes1[0], np.ndarray):
                bboxes1 = [t.xyxy for t in bboxes1]
            bboxes1 = np.array(bboxes1)
            assert bboxes1.ndim == 2 and bboxes1.shape[1] >= 4
        if len(bboxes2) == 0:
            bboxes2 = np.empty([0, 4])
        else:
            if not isinstance(bboxes2[0], np.ndarray):
                bboxes2 = [t.xyxy for t in bboxes2]
