...
class STrack(BaseTrack):
    ...
    def __repr__(self):
        if hasattr(self, "id") and hasattr(self, "state"):
            return f"STrack(id: {self.id}, state: {self.state})"
        else:
            return "STrack(unactivated)"
...
class BYTETracker(BaseTracker):
    ...
    def update(self, dets: np.ndarray, img: np.ndarray = None, embs: np.ndarray = None) -> np.ndarray:
        ...
        dists = 1 - self.asso_func(strack_pool, detections)
        ...
        dists = 1 - self.asso_func(r_tracked_stracks, detections_second)
        ...
        dists = 1 - self.asso_func(unconfirmed, detections)
        # if not self.args.mot20:
        dists = fuse_score(dists, detections)
        ...
        self.active_tracks, self.lost_stracks = self.remove_duplicate_stracks(
    ...
    def remove_duplicate_stracks(self, stracksa, stracksb):
        pdist = 1 - self.asso_func(stracksa, stracksb)
